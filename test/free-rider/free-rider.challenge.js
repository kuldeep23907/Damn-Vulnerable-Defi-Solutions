...
describe('[Challenge] Free Rider', function () {
  ...
  const MARKETPLACE_INITIAL_ETH_BALANCE =
  ...
  const UNISWAP_INITIAL_TOKEN_RESERVE =
    ethers.utils.parseEther('15000');
  const UNISWAP_INITIAL_WETH_RESERVE =
  ...
  before(async function () {
    ...
    expect(
      await this.uniswapPair.balanceOf(deployer.address)
    ).to.be.gt('0');
  ...
  it('Exploit', async function () {
    ...
    this.marketplaceAttack = await (
      await ethers.getContractFactory(
        'FreeRiderNFTMarketplaceAttack',
        deployer
      )
    ).deploy(
    ...
    await this.marketplaceAttack.connect(attacker).getETH();

    await this.marketplaceAttack.connect(attacker).sendToBuyer();
  });
